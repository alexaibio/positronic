...
class TestWorldBroadcast:
    """Tests for broadcast functionality in World."""

    def test_connect_broadcast_with_control_systems(self):
        """Test connect_broadcast with control systems."""
        producer = DummyControlSystem('producer')
        consumer1 = DummyControlSystem('consumer1')
        consumer2 = DummyControlSystem('consumer2')

        with World() as world:
            world.connect_broadcast(
                producer.emitter,
                [consumer1.receiver, consumer2.receiver]
            )
            ...
            msg1 = consumer1.receiver.read()
            msg2 = consumer2.receiver.read()

            assert msg1 is not None and msg2 is not None
            assert msg1.data.value == 5.0
            assert msg2.data.value == 5.0

    def test_start_creates_broadcast_connections(self):
        """Test that start() properly instantiates broadcast connections."""
        producer = DummyControlSystem('producer', steps=1)
        consumer1 = DummyControlSystem('consumer1', steps=1)
        consumer2 = DummyControlSystem('consumer2', steps=1)
        ...
        with World() as world:
            ...
            # Emit shared memory compatible data
            data = DummySMValue(42.0)
            producer.emitter.emit(data, ts=1000)

            list(scheduler)

            # Both consumers should receive the data
            msg1 = consumer1.receiver.read()
            msg2 = consumer2.receiver.read()

            assert msg1 is not None
            assert msg2 is not None
            assert msg1.data.value == 42.0
            assert msg2.data.value == 42.0
            assert msg1.ts == 1000
            assert msg2.ts == 1000
